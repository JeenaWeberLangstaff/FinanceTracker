...
--
-- Host: localhost:3306
-- Generation Time: Apr 27, 2026 at 11:03 AM
...
--
-- Stand-in structure for view `Account_Employee`
...
(2, 'Emma', 'Taylor', 'emma.taylor2@example.com', '9999999999'),
...
--
-- Stand-in structure for view `Account_Public`
-- (See below for the actual view)
--
...
--
-- Structure for view `Account_Employee`
--
DROP TABLE IF EXISTS `Account_Employee`;

CREATE ALGORITHM=UNDEFINED DEFINER=`jpiccoli`@`localhost` SQL SECURITY DEFINER VIEW `Account_Employee`  AS SELECT `Account`.`Account_ID` AS `Account_ID`, `Account`.`User_ID` AS `User_ID`, `Account`.`Bank_name` AS `Bank_name`, `Account`.`Account_type` AS `Account_type`, `Account`.`Balance` AS `Balance` FROM `Account` WHERE (`Account`.`Balance` > 0) ;

-- --------------------------------------------------------

--
-- Structure for view `Account_Public`
--
DROP TABLE IF EXISTS `Account_Public`;

CREATE ALGORITHM=UNDEFINED DEFINER=`jpiccoli`@`localhost` SQL SECURITY DEFINER VIEW `Account_Public`  AS SELECT `Account`.`Account_ID` AS `Account_ID`, `Account`.`User_ID` AS `User_ID`, `Account`.`Bank_name` AS `Bank_name`, `Account`.`Account_type` AS `Account_type` FROM `Account` ;
